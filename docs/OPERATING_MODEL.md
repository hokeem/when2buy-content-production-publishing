...
3. 原帖有效期固定为90分钟；超过后自动标记 `expired`，永不补发。有效期内按 `postedAt` 从新到旧处理，每轮最多2条；热度只能打破相同时间。
4. 保留事件、主体/Ticker、关键数字、事实顺序和信息密度，只轻微重排语言。
5. 核心事实写完即结束；不得添加旧版 When2Buy partner 结尾、替代固定口号或 CTA。
6. 公共文案和图片不得出现来源账号、来源链接、`according to`、`reported by`、`Market radar` 或免责声明。
7. 每条使用生图模型生成完整1:1实体场景，再叠加一次准确 Logo。纯文字卡和通用雷达背景不合格。
8. 制作前和调用Postiz前各检查一次90分钟有效期；过期内容跳过且不阻塞新内容。发布前运行内容标准检查；只有 Postiz 返回 `PUBLISHED` 和公开 X URL 才记录成功。
...
| 任务 | 时间 | 是否写状态 | 输出 |
|---|---|---:|---|
| `when2buy-unified-production` | 每小时 05、15、25、35、45、55 分 | 是，唯一周期写入者 | X内容、状态、内容工厂、数据页 |
