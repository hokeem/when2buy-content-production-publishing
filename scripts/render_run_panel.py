...
def row(post, packages, releases, snapshots):
    ...
    return f'<tr><td data-label="Crawl time">{e(crawl_time(post.get("capturedAt")))}</td><td data-label="Original source">{source}</td><td data-label="when2buy output">{output}</td></tr>'
...
def main():
    ...
    def sections_for(kind):
        ...
        for captured_day in sorted(grouped, reverse=True):
            ...
            sections.append(f'<section class="day-card"><h2>{e(captured_day)}</h2><div class="table-wrap"><table><thead><tr><th>Crawl time</th><th>Original source</th><th>when2buy output</th></tr></thead><tbody>{body}</tbody></table></div></section>')
