...
PERFORMANCE_URL = 'https://reports.aitist.ai/when2buy/performance-dashboard/'
WEEKLY_URL = 'https://reports.aitist.ai/when2buy/weekly-content-analysis/'
...
def latest_attributable(snapshots, post_id, field):
    candidates = [item for item in snapshots.get(str(post_id), []) if item.get(field) is not None and isinstance(item.get('evidence'), dict) and item['evidence'].get('source')]
    return max(candidates, key=lambda item: str(item.get('observedAt', '')), default=None)
...
def row(post, packages, releases, snapshots):
    ...
    post_data = metric_cell(release, snapshots.get(str(release.get('id')), [])) if release else '<span class="muted">Available after publication</span>'
    return f'<tr><td data-label="Crawl time">{e(crawl_time(post.get("capturedAt")))}</td><td data-label="Original source">{source}</td><td data-label="when2buy output">{output}</td><td data-label="Post data">{post_data}</td></tr>'
...
def main():
    ...
    published_posts = [item for item in state.get('posts', []) if item.get('status') == 'published']
    latest_views = [latest_attributable(snapshots, item.get('id'), 'views') for item in published_posts]
    total_views = sum(item['views'] for item in latest_views if item)
    latest_video_views = [latest_attributable(snapshots, item.get('id'), 'videoViews') for item in published_posts]
    video_values = [item['videoViews'] for item in latest_video_views if item]
    video_total = metric(sum(video_values)) if video_values else '—'
    video_note = '' if video_values else '<span>Unavailable: no attributable videoViews returned</span>'
    totals = f'''<section class="totals" aria-label="Run totals"><div><b>{len(state.get('benchmarkPosts', [])):,}</b><span>Crawled benchmark posts</span></div><div><b>{len(state.get('packages', [])):,}</b><span>Produced packages</span></div><div><b>{len(published_posts):,}</b><span>Successful published</span></div><div><b>{total_views:,}</b><span>Cumulative X views<br><em>latest attributable per post</em></span></div><div><b>{video_total}</b><span>Cumulative video views<br>{video_note or '<em>explicit attributable videoViews only</em>'}</span></div></section>'''
