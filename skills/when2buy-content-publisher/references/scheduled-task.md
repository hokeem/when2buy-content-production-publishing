...
Every run stores exact source text, X URL, visible engagement fields when provided, remote original-media URLs, and artifact paths. An agent writes an original when2buy post and creates an original branded visual for every newly eligible source. Where a material claim cannot be independently verified, the package must retain source attribution and the required unverified disclosure, frame it as market radar or broader context, avoid investment advice, and never present the claim as established fact. Publishing still needs action-time confirmation.
